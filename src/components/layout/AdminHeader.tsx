...
export default function AdminHeader({ title, isSidebarOpen, toggleSidebar, showSearch = true, children }: AdminHeaderProps) {
  ...
  return (
    <header className="bg-white border-b border-[#E0E0E0] px-10 flex items-center justify-between sticky top-0 z-40 transition-all font-sans" style={{ minHeight: '110px', paddingTop: 'calc(env(safe-area-inset-top, 0px) + 20px)' }}>
       <div className="flex items-center gap-6">
          ...
          {title ? (
             ...
             <div className="relative w-[300px] xl:w-[400px]">
                ...
                <input 
                  placeholder="Search" 
                  className="w-full h-[38px] rounded-full bg-[#F5F6FA] border border-[#D5D5D5] pl-12 pr-6 text-[14px] font-semibold focus:outline-none"
                />
             </div>
          ) : null}
       </div>
       ...
       <div className="flex items-center gap-4">
          {/* Status Message Hub */}
          <div className="flex items-center gap-3 pr-6 border-r border-[#E0E0E0]">
            <input 
                value={profile.status_message || ""}
                onChange={(e) => handleMessageChange(e.target.value)}
                placeholder="Status mentee hari ini..."
                className="h-[46px] w-[280px] bg-slate-50 border border-slate-100 text-slate-700 text-[13px] font-bold rounded-2xl px-5 outline-none focus:ring-4 ring-accent/10 transition-all placeholder:font-medium placeholder:text-slate-300"
            />
            <select 
              value={profile.manual_status} 
              onChange={(e) => handleStatusChange(e.target.value)}
              className="h-[46px] bg-slate-50 border border-slate-100 text-slate-700 text-[13px] font-bold rounded-2xl px-4 outline-none focus:ring-4 ring-accent/10 cursor-pointer appearance-none hover:bg-white transition-all min-w-[160px] text-center"
            >
          ...
          <div className="flex items-center gap-4 pl-2">
             ...
             <div className="w-[48px] h-[48px] rounded-2xl overflow-hidden border-2 border-slate-100 shadow-sm relative">
