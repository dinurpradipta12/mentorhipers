   ...
   return (
      <div className="min-h-screen bg-[#F8FAFC] pb-20">
         ...
         <div className="max-w-[1600px] mx-auto p-6 md:p-10 lg:p-14 space-y-12">
                                       ...
                                       <div className="relative shrink-0">
                                          ...
                                          >
                                             {m.v2_profiles?.avatar_url ? (
                                                <img src={m.v2_profiles.avatar_url} alt={m.v2_profiles?.full_name} className="w-full h-full object-contain scale-[0.8]" />
                                             ) : (
            ...
            {activeTab === 'profile' && (
               <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-12 max-w-4xl mx-auto py-10">
                  <div className="flex flex-col items-center text-center space-y-8">
                     <div className="relative">
                        <div className="w-48 h-48 flex items-center justify-center relative">
                           {currentUser?.avatar_url ? (
                              <img src={currentUser.avatar_url} alt={currentUser?.full_name} className="w-full h-full object-contain scale-[1.3]" />
                           ) : (
                              <div className="w-full h-full flex items-center justify-center font-black text-6xl text-slate-300">{currentUser?.full_name?.charAt(0)}</div>
                           )}
                        </div>
                        <button
                           onClick={async () => {
                           ...
                           className="absolute bottom-4 -left-4 w-12 h-12 bg-rose-500 text-white rounded-2xl shadow-xl border-4 border-white flex items-center justify-center hover:scale-110 active:scale-95 transition-all z-20"
                           title="Logout"
                        >
                        ...
                        <button
                           onClick={() => setIsPhotoSetupOpen(true)}
                           className="absolute bottom-4 -right-4 w-12 h-12 bg-blue-600 text-white rounded-2xl shadow-xl border-4 border-white flex items-center justify-center hover:scale-110 active:scale-95 transition-all z-20"
                           title="Edit Avatar"
                        >
                     ...
                     <div className="space-y-2">
                        <h3 className="text-5xl font-black text-slate-900 tracking-tighter">{currentUser?.full_name}</h3>
                        <div className="inline-flex items-center gap-3 px-6 py-2 rounded-full bg-blue-50 border border-blue-100 text-blue-600">
                           <User size={14} />
                           <span className="text-[11px] font-black uppercase tracking-[0.2em]">{students.find((s: any) => s.profile_id === currentUser?.id)?.group_name || 'Academic Scholar'}</span>
                        </div>
                     </div>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                     <div className="bg-white rounded-[40px] p-10 border border-slate-100 shadow-xl shadow-slate-200/50 space-y-6">
                        <h4 className="text-sm font-black text-slate-900 uppercase tracking-widest">Digital Identity Card</h4>
                        <div onClick={() => setIsIdCardOpen(true)} className="relative aspect-[1.6/1] bg-gradient-to-br from-[#0ea5e9] to-[#1e3a8a] rounded-[32px] shadow-2xl overflow-hidden cursor-pointer group hover:scale-[1.02] active:scale-95 transition-all">
                           <div className="absolute inset-0 p-8 flex items-center justify-between">
                              <div className="space-y-4">
                                 <div className="w-24 h-12 flex items-center justify-start"><img src="/logo_rs.png" className="w-full h-full object-contain brightness-0 invert opacity-40" alt="RS" /></div>
                                 <p className="text-white/40 text-[9px] font-black uppercase tracking-[0.4em]">Official Student Card</p>
                              </div>
                              <div className="w-24 h-24 rounded-3xl bg-white/10 backdrop-blur-md border border-white/20 flex items-center justify-center overflow-hidden rotate-6">
                                 {currentUser?.avatar_url ? <img src={currentUser.avatar_url} alt="Avatar" className="w-full h-full object-contain scale-[1.2]" /> : <QrCode size={40} className="text-white/20" />}
                              </div>
                           </div>
                        </div>
                        <p className="text-[10px] font-bold text-slate-400 text-center uppercase tracking-widest">Click to view & download identity card</p>
                     </div>

                     <div className="bg-white rounded-[40px] p-10 border border-slate-100 shadow-xl shadow-slate-200/50 space-y-6">
                        <h4 className="text-sm font-black text-slate-900 uppercase tracking-widest">Student Information</h4>
                        <div className="space-y-4">
                           {[
                              { label: 'Full Name', value: currentUser?.full_name },
                              { label: 'Role', value: currentUser?.role || 'Student' },
                              { label: 'Batch', value: batch?.name },
                              { label: 'Status', value: 'Active Authorized' }
                           ].map(item => (
