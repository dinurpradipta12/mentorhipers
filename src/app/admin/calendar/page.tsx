...
export default function AdminCalendarPage() {
  ...
  useEffect(() => {
    ...
    const cachedFavicon = localStorage.getItem('app_favicon'); // Added favicon cache load
            ...
            <div className="col-span-12 lg:col-span-4 xl:col-span-3">
              <Card className="p-8 h-full bg-white border-none shadow-[0px_10px_30px_rgba(0,0,0,0.02)] rounded-[14px] flex flex-col gap-8 text-left">
                ...
                <div className="space-y-6">
                  <div className="flex items-center justify-between">
                    <h4 className="text-[16px] font-extrabold text-[#202224] flex items-center gap-2">
                       <Clock className="w-4 h-4 text-[#4880FF]"/> Upcoming Schedule
                    </h4>
                    <span className="px-2 py-0.5 rounded-lg bg-[#4880FF]/10 text-[#4880FF] text-[10px] font-bold">Today</span>
                  </div>
                  <div className="space-y-6 max-h-[500px] overflow-y-auto no-scrollbar pr-2">
                    {allEvents.slice(0, 10).map((ev, idx) => (
                      <div key={idx} className="flex gap-4 items-start group">
                        <div className="w-[48px] h-[48px] rounded-full bg-slate-100 overflow-hidden flex-shrink-0 border-2 border-white shadow-sm group-hover:border-[#4880FF]/50 transition-all">
                          <img src={ev.clientAvatar || `https://i.pravatar.cc/150?u=${ev.clientId}`} className="w-full h-full object-cover" />
                        </div>
                        <div className="space-y-1 flex-1">
                          <p className="text-[14px] font-extrabold text-[#202224] leading-tight group-hover:text-[#4880FF] transition-all">{ev.title}</p>
                          <div className="flex items-center gap-2">
                             ...
                             <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: ev.clientColor }} />
                          </div>
                        </div>
                      </div>
                    ))}
                  </div>
                  <Button variant="ghost" className="w-full h-[48px] bg-[#F5F6FA] text-[#202224] font-bold text-[13px] rounded-xl hover:bg-[#E0E0E0]">View Full Timeline</Button>
                </div>
              </Card>
            </div>
